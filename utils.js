...
export const lexer = (code) => {
  ...
  return lines.map(line => line.split(/ +(?=(?:(?:[^"]*"){2})*[^"]*$)/g).filter((l) => l.length > 0).map((token) => {
    if (!isNaN(token)) return ({ type: TOKEN_TYPES.NUMBER, value: parseInt(token) })
    ...
    if (token.includes("\"")) {
      ...
      if (tokenOutsideQuotes.includes("$")) {
        return ({
          type: TOKEN_TYPES.VAR_STRING, value: clearToken(token)
        })
      } else {
        return ({
          type: TOKEN_TYPES.STRING, value: clearToken(token)
        })
      }
    } else if (token.includes("%")) {
      return ({
        type: TOKEN_TYPES.VAR_NUMBER, value: clearToken(token)
      })
    } else {
        ...
        switch (current_token.type) {
              ...
              switch (current_token.value) {
                case RESERVED_WORDS.PRINT: {
                  ...
                  if (argument.type === TOKEN_TYPES.WORD) {
                    ...
                    if (!variable) throw new Error(`Variable ${argument.value} cannot be accessed before initialization.`)
                  ...
                  if (argument.type === TOKEN_TYPES.NUMBER || argument.type === TOKEN_TYPES.STRING) {
                    expression.arguments.push({
                      type: 'NumberOrString',
                      value: argument.value
                    })
                  ...
                  throw new Error('PRINT command must be followed by a number/string/variable.')
                }
                ...
                case RESERVED_WORDS.CLS: {
                  ...
                  if (tokens.length > 0) {
                    throw new Error('CLS command must not be followed by anything.')
                  }
                ...
                case RESERVED_WORDS.END: {
                  ...
                  if (tokens.length > 0) {
                    throw new Error('END command must not be followed by anything.')
                  }
                ...
                case RESERVED_WORDS.GOTO: {
                  ...
                  if (argument && argument.type === TOKEN_TYPES.NUMBER && tokens.length === 0) {
                    ...
                    throw new Error('GOTO command must be followed only by a number.')
                  }
                  break
                }
                ...
                case RESERVED_WORDS.IF: {
                  ...
                  console.log({ conditionToken })
                  if (conditionToken && conditionToken.type === TOKEN_TYPES.WORD || conditionToken.type === TOKEN_TYPES.STRING) {
                    if (!COMPARATORS.some(comparator => conditionToken.value.includes(comparator))) {
                      throw new Error('IF command must have a condition with a comparator.')
                    }
                    expression.arguments.push({
                      type: 'Condition',
          ...
          case TOKEN_TYPES.VAR_NUMBER: {
            ...
            if (splittedValue.length === 2) {
              ...
              if (isNotANumber) throw new Error(`Variable value of type NUMBER unexpected for variable named '${splittedValue[0]}'`)

              globalVariables = { ...globalVariables, [splittedValue[0]]: splittedValue[1] }
            } else {
              throw new Error(`Variable declaration unexpected for variable named '${splittedValue[0]}'`)
            }
            break
          }
          ...
          case TOKEN_TYPES.VAR_STRING: {
            ...
            if (splittedValue.length === 2) {
              const isNotANumber = isNaN(splittedValue[1])
              if (!isNotANumber) throw new Error(`Variable value of type STRING unexpected for variable named '${splittedValue[0]}'`)
  ...
  try {
    ...
    while (index !== -1) {
      ...
      if (result && !isNaN(result)) {
        ...
        if (!indexFound) throw new Error(`Not possible to go to line ${result}, not found`)
  ...
  content.forEach(statement => {
    ...
    switch (name) {
      case RESERVED_WORDS.PRINT:
        ...
        const value = argument.type === 'Variable' ? globalVariables[argument.value] : argument.value
      ...
      case RESERVED_WORDS.IF:
        ...
        if (conditionResult) {
          if (args[1] && args[1].value && !isNaN(args[1].value)) result = args[1].value;
        }
        break
    }
  });
...
const performCondition = (conditionToken) => {
  ...
  let comparator = COMPARATORS.find(comparator => conditionValue.includes(comparator))
